...
import br.com.fiap.postech.gestao_restaurantes.usecase.validarLogin.handler.CampoNaoVazioHandler;
import br.com.fiap.postech.gestao_restaurantes.usecase.validarLogin.handler.LoginExistenteHandler;
import br.com.fiap.postech.gestao_restaurantes.usecase.validarLogin.handler.AutenticarUsuarioHandler;
...
public class AutenticarUsuarioUsecaseImpl implements AutenticarUsuarioUsecase {
    ...
    public Boolean executar(Credenciais credenciais) {
        AutenticarUsuarioHandler chain = new CampoNaoVazioHandler()
                .setNext(new LoginExistenteHandler(usuarioGateway)
                        .setNext(new SenhaCorretaHandler()));

        return chain.handle(credenciais);
    }
}
